...
class GnuFCompiler(FCompiler):
    ...
    def get_flags_opt(self):
        ...
        if v and v<='3.3.3':
            ...
            if is_win32():
                # -mincoming-stack-boundary=2 reverts the
                # change to 16 byte stack alignment since GCC 4.6
                # Windows ABI 32 bit defines 4 bytes stack alignment
                # -ffpe-summary=invalid,zero: don't show overflow, 
                # underflow and denormal floating point exception warnings
                opt = ['-O2 -march=pentium4 -mtune=generic -mfpmath=sse -msse2 -mincoming-stack-boundary=2 -ffpe-summary=invalid,zero'] 
            else:
                opt = ['-O2 -march=x86-64 -DMS_WIN64 -mtune=generic -msse2 -ffpe-summary=invalid,zero'] 
        else:
            opt = ['-O2'] 
        # opt.append()
        return opt
