...
if __SCIPY_SETUP__:
    ...
    if _sys.platform == 'win32':
        from scipy.__config__ import get_info as _get_info
        _blas_opt = _get_info('blas_opt')
        if 'libraries' in _blas_opt and 'openblaspy' in _blas_opt['libraries']:
            from numpy.__config__ import get_info as _np_get_info
            _np_blas_opt = _np_get_info('blas_opt')
            if not ('libraries' in _np_blas_opt and 'openblaspy' in _np_blas_opt['libraries']):
                del _np_blas_opt, _np_get_info
                raise ImportError("Numpy OpenBLAS flavour is needed for this scipy build.")
            del _np_blas_opt, _np_get_info
        del _blas_opt, _get_info, _sys
